...
def overplot_mask(ax,
    ...
    for i in range(len(mask[:, 0])):
        for j in range(len(mask[0, :])):
            if mask[i, j]:
                ax.add_patch(mpl.patches.Rectangle((j-0.5, i-0.5), 1, 1, edgecolor=ec,
                                                   facecolor=fc, linewidth=lw, alpha=alpha))
...
def plot_periodogram(ax,
    ...
    # Plot peaks
    if len(ind) > 0:
        ax.scatter(x[mask][ind], y[mask][ind], marker='o', c='red', s=2, label=f'SNR > {snr}',
                   rasterized=False, zorder=0, edgecolors='None', linewidth=0.1)
    # Plot background
    ...
    ax.legend(ncol=1, loc='best', fontsize=0.75*fsize, frameon=False, handletextpad=0.5, markerscale=1.5,
              handlelength=1)
...
def plot_sector(sectorInfo,
    ...
    def plot_images(fig,
        ...
        def plot_median_image(ax,
            ...
            ax.set_title('Median', size=TitleFontSize, pad=2)
        ...
        def plot_neighbourhood(ax,
            ...
            ax.scatter(target.pix[:, 0], target.pix[:, 1], s=sizes, c='r', edgecolors='k',
                       linewidth=0.2, zorder=6, label=f'{target.mag:.1f}')
            if neighbours.pix.size > 0:
                sizes = scalesymbols(neighbours.mag, np.amin(mags), np.amax(mags), scale=15.)
                ax.scatter(neighbours.pix[:, 0], neighbours.pix[:, 1], s=sizes, c='w',
    ...
    if not ok:
        ...
        ax.text(0.5, 0.5, text, transform=ax.transAxes, fontsize=1.1*fsize, ha='center', va='center', color='green',
                wrap=True)
    else:
        # Plot time series
...
def plot_diagnosis(sectorInfo,
    ...
    # Validate input
    error_message = ('`sectorInfo` must be a str or pathlib.Path pointing to the pickle file created by ',
                     '`extract_light_curve` or `group_lcs`. Altervatively, it can also be the SimpleNamespace, ',
                     'or a list of those, cointained in the pickle file.')
    if isinstance(sectorInfo, SimpleNamespace):
        alt_pdfname = f'diagnostic_plot_TIC{sectorInfo.tic}_sector{sectorInfo.sector}.pdf'
        pdfname = pdfname if isinstance(pdfname, str) else alt_pdfname
        ...
        if all([isinstance(info, SimpleNamespace) for info in sectorInfo]):
            alt_pdfname = f'diagnostic_plot_TIC{sectorInfo[0].tic}.pdf'
            pdfname = pdfname if isinstance(pdfname, str) else alt_pdfname
    ...
    if len(lcs) >= 1:
        # Print status
        if verbose:
            print("Generating plot of stitched light curve and periodogram.")
